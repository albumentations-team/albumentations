...
__all__ = ["Scale3d", "RotateAroundAxis3d", "Crop3d", "RandomMove3d", "Center3d", "RandomDropout3d", "Flip3d"]
...
class RotateAroundAxis3d(PointCloudsTransform):
    ...
    def get_transform_init_args(self):
        return {"rotation_limit": to_tuple(self.rotation_limit, bias=0), "rotation_axis": self.axis}
...
class Move3d(PointCloudsTransform):
    ...
    def apply(self, points, **params):
        return F.move(points, self.offset)


class RandomMove3d(PointCloudsTransform):
    ...
    def __init__(self, x_min=-1.0, y_min=-1.0, z_min=-1.0, x_max=1.0, y_max=1.0, z_max=1.0, always_apply=False, p=0.5):
        super().__init__(always_apply, p)
        self.x_min = x_min
        self.y_min = y_min
        self.z_min = z_min

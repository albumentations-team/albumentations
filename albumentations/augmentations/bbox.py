...
def normalize_bbox(bbox, rows, cols):
    """
    Normalizes coordinates of a bounding box. Divides x-coordinates by image width and y-coordinates by image height.
    """
...
def denormalize_bbox(bbox, rows, cols):
    """
    Denormalizes coordinates of a bounding box. Multiplies x-coordinates by image width and y-coordinates
...
def denormalize_bboxes(bboxes, rows, cols):
    """
    Denormalizes a list of bounding boxes.
    """
...
def calculate_bbox_area(bbox, rows, cols):
    """
    Calculates the area of a bounding box in pixels.
    """
...
def convert_bbox_to_albumentations(img, bbox, source_format):
    """Converts a bounding box from a format specified in `source_format` to the format used by albumentations:
...
def convert_bbox_from_albumentations(img, bbox, target_format):
    """Converts a bounding box from the format used by albumentations to a format, specified in `target_format`.
...
def convert_bboxes_to_albumentations(img, bboxes, source_format):
    """
    Converts a list bounding boxes from a format specified in `source_format` to the format used by albumentations
    """
...
def convert_bboxes_from_albumentations(img, bboxes, target_format):
    """
    Converts a list of bounding boxes from the format used by albumentations to a format, specified in `target_format`.
    """

...
def check_keypoint(keypoint, rows, cols):
    """Check if keypoint coordinates are less than image shapes"""
    kp = KeyPoint(*keypoint)
    for name, value, size in zip(["x", "y"], [kp.x, kp.y], [cols, rows]):
        if not 0 <= value < size:
            raise ValueError(

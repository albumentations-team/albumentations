...
def mosaic4(
    image_batch: List[np.ndarray], height: int, width: int, value: Optional[ImageColorType] = None
) -> np.ndarray:
...
def bbox_mosaic4(
    bbox: BoxInternalType, rows: int, cols: int, position_index: int, height: int, width: int
) -> BoxInternalType:
...
def keypoint_mosaic4(
    keypoint: KeypointInternalType, rows: int, cols: int, position_index: int, height: int, width: int
) -> KeypointInternalType:
    """Put the given bbox in one of the cells of the 2x2 grid.
    Args:
        keypoint (tuple): A keypoint `(x, y, angle, scale)`.

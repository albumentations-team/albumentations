...
def elastic_transform_v2(
    ...
    Differences are as follows:
     1. Return ``matrix``, ``dx`` and ``dy`` for calculate keypoint after affine and elastic transform.
     2. Do not use ``@preserve_shape`` as modifier to ensure return multiple values.
    ...
    return remap_fn(img).reshape(img.shape), matrix, dx, dy

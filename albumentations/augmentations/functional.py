...
def fourier_domain_adaptation(img, target_img, beta):
    ...
    amplitude_src_shifted = np.fft.fftshift(amplitude_src, axes=(0, 1))
    amplitude_trg_shifted = np.fft.fftshift(amplitude_trg, axes=(0, 1))

    height, width, _ = amplitude_src_shifted.shape
    border = (np.floor(np.min((height, width)) * beta)).astype(int)
    center_x, center_y = np.floor(height / 2.0).astype(int), np.floor(width / 2.0).astype(int)

    y1, y2 = center_y - border, center_y + border + 1
    x1, x2 = center_x - border, center_x + border + 1

    amplitude_src_shifted[y1:y2, x1:x2] = amplitude_trg_shifted[y1:y2, x1:x2]
    amplitude_src_mutated = np.fft.ifftshift(amplitude_src_shifted, axes=(0, 1))

    # mutated fft of source
    fft_src_mutated = amplitude_src_mutated * np.exp(1j * phase_src)

    # get mutated image
    src_image_transformed = np.fft.ifft2(fft_src_mutated, axes=(0, 1))

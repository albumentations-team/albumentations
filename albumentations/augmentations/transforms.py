...
from albumentations.augmentations.functional import get_roi_box, random_gaze_img
...
class Crop(DualTransform):
    ...
    def __init__(self, x_min=0, y_min=0, x_max=1024, y_max=1024, always_apply=False, p=1.0):
        uper(Crop, self).__init__(always_apply, p)
        self.x_min = x_min
        self.y_min = y_min
        self.x_max = x_max
        self.y_max = y_max
...
class RandomGaze(DualTransform):
    """Select a random subsample of all boxes, create a box that encompasses all those boxes and jitter
        it randomly. Therefore, it only works with images that have boxes.

    Args:
        view_min (int): Minimal amount of boxes in field of view.
        context_min (float): Minimal additional area around focused boxes.
        context_max (float): Maximal additional area around focused boxes.
        max_size (int): recenter initial gaze to have at most this number of pixels (limit GPU mem use)
    ...
    def __init__(self, view_min=2, context_min=0.2, context_max=0.9, max_size=None, always_apply=False, p=1.0):
        super(RandomGaze, self).__init__(always_apply, p)
        self.roi_box = None
    ...
    def apply(self, img, **params):
        if params['roi_box'] is None:
            return img
        else:
            return random_gaze_img(img, params['roi_box'])

    def apply_to_bbox(self, bbox, **params):
        if params['roi_box'] is None:
            return bbox
        else:
            return F.random_gaze_bbox(bbox=bbox, roi_box=params['roi_box'],
    ...
    def apply_to_mask(self, mask, **params):
        if params['roi_box'] is None:
            ...
            return random_gaze_img(mask, params['roi_box'])

    def get_params_dependent_on_targets(self, params):
        roi_box = get_roi_box(bboxes=params['bboxes'], rows=params['image'].shape[0], cols=params['image'].shape[1],
                              view_min=self.view_min, context_min=self.context_min, context_max=self.context_max,
                              max_size=self.max_size)
        return {'roi_box': roi_box}

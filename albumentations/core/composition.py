...
from .bbox_utils import BboxBatchProcessor, BboxParams, BboxProcessor
from .keypoints_utils import KeypointParams, KeypointsBatchProcessor, KeypointsProcessor
...
class Compose(BaseCompose):
    ...
    def __init__(
        ...
        self.processors: typing.Dict[str, typing.Union[DataProcessor]] = {}
        ...
        if bbox_params:
            ...
            self.processors["bboxes"] = self._get_bbox_processor(b_params, additional_targets)
        ...
        if keypoint_params:
            ...
            self.processors["keypoints"] = self._get_keypoints_processor(k_params, additional_targets)
    ...
    def _make_targets_contiguous(self, data: typing.Dict[str, typing.Any]) -> typing.Dict[str, typing.Any]:
        result = {}
...
class BatchCompose(Compose):
    ...
    def _check_data_post_transform(self, batched_data: typing.Dict[str, typing.Any]) -> typing.Dict[str, typing.Any]:
        ...
        for data in datalist:
            ...
            for p in self.processors.values():
                if not getattr(p.params, "check_each_transform", False):
                    continue
                p = typing.cast(typing.Union[BboxBatchProcessor, KeypointsBatchProcessor], p)
    ...
    def _check_args(self, additional_targets, **kwargs) -> None:
        ...
        unbatched_targets = {to_unbatched_name(k): to_unbatched_name(v) for k, v in additional_targets.items()}
        for data in datalist:
            super(BatchCompose, self)._check_args(unbatched_targets, **data)

    def _make_targets_contiguous(self, batched_data: typing.Dict[str, typing.Any]) -> typing.Dict[str, typing.Any]:
        datalist = batch2list(batched_data)
        if len(datalist) == 0:
            return batched_data
        processed = []
...
class ForEach(BaseCompose):
    ...
    ```
        for i in range(batch_size):
            for t in transforms:
                processed = t({"image": data["image_batch"][i], "bboxes": data["bboxes_batch"][i], ...})
                data["image_batch"][i] = processed["image"]
                ...
        return data
    ...
    def __call__(self, *args, force_apply: bool = False, **batched_data) -> typing.Dict[str, typing.List]:
        datalist = batch2list(batched_data)
        processed = []
        for data in datalist:
            for t in self.transforms:
...
class Repeat(BaseCompose):
    ...
    Internally, this container works as the following way:
    ```
        out_data = {"image_batch": [], ...}
        for _ in range(n):
            data = in_data
            for t in transforms:
                data = t(**data)
            out_data["image_batch"] += data["image_batch"]
            ...
        return out_data
    ```

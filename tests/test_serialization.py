...
def test_serialization_v2_conversion_without_totensor():
    ...
    transform_0_4_6 = A.load(
        os.path.join(files_directory, "transform_v0.4.6_without_totensor.json")
    )
    with open(
        os.path.join(files_directory, "output_v0.4.6_without_totensor.json")
    ) as f:
...
def test_serialization_v2_conversion_with_totensor():
    ...
    files_directory = os.path.join(current_directory, "files")
    transform_0_4_6 = A.load(
        os.path.join(files_directory, "transform_v0.4.6_with_totensor.json")
    )

...
def test_augmentations_serialization(augmentation_cls, params, p, seed, image, mask, always_apply):
    ...
    aug_data = aug(random_state=random_state, image=image, mask=mask)
    random_state = RandomState(seed)
    deserialized_aug_data = deserialized_aug(random_state=random_state, image=image, mask=mask)
...
def test_augmentations_for_bboxes_serialization(augmentation_cls, params, p, seed, image, bboxes, always_apply):
    ...
    deserialized_aug = A.from_dict(serialized_aug)
    random_state = RandomState(seed)
    aug_data = aug(random_state=random_state, image=image, bboxes=bboxes)
    random_state = RandomState(seed)
    deserialized_aug_data = deserialized_aug(random_state=random_state, image=image, bboxes=bboxes)
    assert np.array_equal(aug_data['image'], deserialized_aug_data['image'])
...
def test_augmentations_for_keypoints_serialization(augmentation_cls, params, p, seed, image, keypoints, always_apply):
    ...
    deserialized_aug = A.from_dict(serialized_aug)
    random_state = RandomState(seed)
    aug_data = aug(random_state=random_state, image=image, keypoints=keypoints)
    random_state = RandomState(seed)
...
def test_image_only_crop_around_bbox_augmentation_serialization(p, seed, image, always_apply):
    ...
    deserialized_aug = A.from_dict(serialized_aug)
    random_state = RandomState(seed)
    aug_data = aug(random_state=random_state, **annotations)
    random_state = RandomState(seed)
    deserialized_aug_data = deserialized_aug(random_state=random_state, **annotations)
    assert np.array_equal(aug_data['image'], deserialized_aug_data['image'])
...
def test_transform_pipeline_serialization_with_bboxes(seed, image, bboxes, bbox_format, labels):
    ...
    deserialized_aug = A.from_dict(serialized_aug)
    random_state = RandomState(seed)
    aug_data = aug(random_state=random_state, image=image, bboxes=bboxes, labels=labels)
    random_state = RandomState(seed)
    deserialized_aug_data = deserialized_aug(random_state=random_state, image=image, bboxes=bboxes, labels=labels)
    assert np.array_equal(aug_data['image'], deserialized_aug_data['image'])
    assert np.array_equal(aug_data['bboxes'], deserialized_aug_data['bboxes'])
...
def test_additional_targets_for_image_only_serialization(augmentation_cls, params, image, seed):
    ...
    deserialized_aug = A.from_dict(serialized_aug)
    random_state = RandomState(seed)
    aug_data = aug(random_state=random_state, image=image, image2=image2)
    random_state = RandomState(seed)
    deserialized_aug_data = deserialized_aug(random_state=random_state, image=image, image2=image2)
    assert np.array_equal(aug_data['image'], deserialized_aug_data['image'])
...
def test_lambda_serialization(image, mask, bboxes, keypoints, seed, p):
    ...
    random_state = RandomState(seed)
    aug_data = aug(random_state=random_state, image=image, mask=mask, bboxes=bboxes, keypoints=keypoints)
    random_state = RandomState(seed)
    deserialized_aug_data = deserialized_aug(random_state=random_state, image=image, mask=mask, bboxes=bboxes,
                                             keypoints=keypoints)
    assert np.array_equal(aug_data['image'], deserialized_aug_data['image'])
    assert np.array_equal(aug_data['mask'], deserialized_aug_data['mask'])
    assert np.array_equal(aug_data['bboxes'], deserialized_aug_data['bboxes'])

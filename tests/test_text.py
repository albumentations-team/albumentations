...
@pytest.mark.parametrize(
    "font_color, expected_background",
    [
        ((0, 0, 0), (255, 255, 255)),  # Black font color in RGB
        ((255, 255, 255), (0, 0, 0)),  # White font color in RGB
        ((127, 127, 127), (128, 128, 128)),  # Gray font color in RGB
        (0, 255),  # Black font color in Grayscale
        (255, 0),  # White font color in Grayscale
    ]
)
def test_compute_temp_background_color(font_color, expected_background):
    background_color = ftext.compute_temp_background_color(font_color)
    assert background_color == expected_background


@pytest.mark.parametrize(
    "bbox_shape, text, font, font_color, expected_background",
    [
        ((100, 100), "Test", ImageFont.load_default(), (0, 0, 0), (255, 255, 255)),  # Black font color in RGB
        ((100, 100), "Test", ImageFont.load_default(), (255, 255, 255), (0, 0, 0)),  # White font color in RGB
        ((100, 100), "Test", ImageFont.load_default(), (127, 127, 127), (128, 128, 128)),  # Gray font color in RGB
        ((100, 100), "Test", ImageFont.load_default(), 0, 255),  # Black font color in Grayscale
        ((100, 100), "Test", ImageFont.load_default(), 255, 0),  # White font color in Grayscale
    ]
)
def test_render_text(bbox_shape, text, font, font_color, expected_background):
    final_bbox_img_np, mask = ftext.render_text(bbox_shape, text, font, font_color)

    assert final_bbox_img_np.shape[:2] == bbox_shape
    if isinstance(font_color, tuple):
        assert final_bbox_img_np.shape[2] == 3
        assert (final_bbox_img_np[0, 0] == expected_background).all()
    elif isinstance(font_color, int):
        assert final_bbox_img_np.ndim == 2
        assert final_bbox_img_np[0, 0] == expected_background

    # Check that the mask is correctly identifying the text
    assert mask.shape == bbox_shape
    assert mask.dtype == np.uint8

    # Ensure mask has non-zero values where the text is supposed to be
    assert np.any(mask != 0)

    # Ensure outside of the text, only the background color is present
    if isinstance(font_color, tuple):
        assert np.all(final_bbox_img_np[mask == 0] == np.array(expected_background))
    elif isinstance(font_color, int):
        assert np.all(final_bbox_img_np[mask == 0] == expected_background)

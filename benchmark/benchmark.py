...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Augmentation libraries performance benchmark"
    )
    parser.add_argument(
        "-d",
        "--data-dir",
...
class MarkdownGenerator:
    ...
    def _make_versions_text(self):
        ...
        return "Python and library versions: {}.".format(
            ", ".join(libraries_with_versions)
        )
    ...
    def print(self):
        ...
        writer.styles = [Style(align="left")] + [
            Style(align="center") for _ in range(len(writer.headers) - 1)
        ]
        writer.write_table()
...
class HorizontalFlip(BenchmarkTest):
    def __init__(self):
        ...
        self.augmentor_op = Operations.Flip(
            probability=1, top_bottom_left_right="LEFT_RIGHT"
        )
...
class VerticalFlip(BenchmarkTest):
    def __init__(self):
        ...
        self.augmentor_op = Operations.Flip(
            probability=1, top_bottom_left_right="TOP_BOTTOM"
        )
...
class Rotate(BenchmarkTest):
    def __init__(self):
        ...
        self.augmentor_op = Operations.RotateStandard(
            probability=1, max_left_rotation=45, max_right_rotation=45
        )
        self.solt_stream = slc.Stream(
            [slt.Rotate(p=1, angle_range=(45, 45))], padding="r"
        )
    ...
    def keras(self, img):
        img = keras.apply_affine_transform(
            img, theta=45, channel_axis=2, fill_mode="reflect"
        )
...
class Brightness(BenchmarkTest):
    def __init__(self):
        ...
        self.augmentor_op = Operations.RandomBrightness(
            probability=1, min_factor=1.5, max_factor=1.5
        )
        self.solt_stream = slc.Stream(
    ...
    def albumentations(self, img):
        return albumentations.brightness_contrast_adjust(
            img, beta=0.5, beta_by_max=True
        )
...
class Contrast(BenchmarkTest):
    def __init__(self):
        ...
        self.augmentor_op = Operations.RandomContrast(
            probability=1, min_factor=1.5, max_factor=1.5
        )
...
class BrightnessContrast(BenchmarkTest):
    ...
    def albumentations(self, img):
        return albumentations.brightness_contrast_adjust(
            img, alpha=1.5, beta=0.5, beta_by_max=True
        )
...
class ShiftScaleRotate(BenchmarkTest):
    ...
    def torchvision_transform(self, img):
        return torchvision.affine(
            img, angle=45, translate=(50, 50), scale=2, shear=0, resample=Image.BILINEAR
        )

    def keras(self, img):
        img = keras.apply_affine_transform(
            img, theta=45, tx=50, ty=50, zx=0.5, zy=0.5, fill_mode="reflect"
        )
        return np.ascontiguousarray(img)
...
class RandomCrop64(BenchmarkTest):
    def __init__(self):
        ...
        self.augmentor_op = Operations.Crop(
            probability=1, width=64, height=64, centre=False
        )
...
class RandomSizedCrop_64_512(BenchmarkTest):
    def __init__(self):
        ...
        self.augmentor_pipeline.add_operation(
            Operations.Resize(
                probability=1, width=512, height=512, resample_filter="BILINEAR"
            )
        )
        self.imgaug_transform = iaa.Sequential(
            [
...
class PadToSize512(BenchmarkTest):
    ...
    def torchvision_transform(self, img):
        if img.size[0] < 512:
            img = torchvision.pad(
                img, (int((1 + 512 - img.size[0]) / 2), 0), padding_mode="reflect"
            )
        if img.size[1] < 512:
            img = torchvision.pad(
                img, (0, int((1 + 512 - img.size[1]) / 2)), padding_mode="reflect"
            )
        return img
...
class Resize512(BenchmarkTest):
    def __init__(self):
        ...
        self.augmentor_op = Operations.Resize(
            probability=1, width=512, height=512, resample_filter="BILINEAR"
        )
...
class MultiplyElementwise(BenchmarkTest):
    def __init__(self):
        self.aug = A.MultiplicativeNoise(
            (0, 1), per_channel=True, elementwise=True, p=1
        )
